	...
	$.fn.fullpage = function(options) {
		...
		*/
		...
		$.fn.fullpage.moveSectionUp = function(){
			...
			if (prev.length) {
				$('.footer_w').stop().animate({"bottom":"-92px"});
		...
		$.fn.fullpage.moveSectionDown = function (){
			...
			if(next.length == 0){
				var fullTop = $('#fullpage').css("top");
				var newfullTop = parseInt(fullTop)-92;
				var n1 = parseInt(fullTop);
				var sectionL = $('.section').length-1;
				var vh = $(window).height();
				$('.footer_w').stop().animate({"bottom":"0px"});
				if(n1 == -sectionL*vh || n1 == -(sectionL*vh+160) || n1 > -sectionL*vh){
					$('#fullpage').stop().animate({"top":newfullTop});
				}
			}
